...
from entity.managers import SpecialDateManager
...
class Location(models.Model):
    ...
    is_phone = models.BooleanField(_('is phone'), default=False,help_text=_("Only used for Phone"))
    is_street_address = models.BooleanField(_('is street address'),default=False, help_text=_("Only used for Street Address"))
...
class PhoneNumber(models.Model):
    """Phone Number model."""
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE, limit_choices_to={'app_label': 'entity'})
    object_id = models.IntegerField(db_index=True)
    content_object = GenericForeignKey()

    phone_number = models.CharField(_('number'), max_length=50)
    location = models.ForeignKey(Location, on_delete=models.CASCADE, limit_choices_to={'is_street_address': False})
...
class EmailAddress(models.Model):
    ...
    location = models.ForeignKey(Location, on_delete=models.CASCADE, limit_choices_to={'is_street_address': False, 'is_phone': False})
...
class InstantMessenger(models.Model):
    ...
    location = models.ForeignKey(Location, on_delete=models.CASCADE, limit_choices_to={'is_street_address': False, 'is_phone': False})
    service = models.CharField(_('service'), max_length=11,choices=IM_SERVICE_CHOICES, default=OTHER)
...
class WebSite(models.Model):
    ...
    def get_absolute_url(self):
        return "%s?web_site=%s" % (self.content_object.get_absolute_url(),self.pk)
...
class StreetAddress(models.Model):
    ...
    # Linked Fields
    location = models.ForeignKey(Location, on_delete=models.CASCADE, limit_choices_to={'is_phone': False})
...
class Company(models.Model):
    ...
    @permalink
    def get_absolute_url(self):
        return ('company_detail', None, {
            'pk': self.pk,
        })

    @permalink
    def get_update_url(self):
        return ('company_update', None, {
            'pk': self.pk,
        })

    @permalink
    def get_delete_url(self):
        return ('company_delete', None, {
            'pk': self.pk,
        })
...
class Person(models.Model):
    ...
    user = models.OneToOneField(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, blank=True, null=True, verbose_name=_('user'))
    #note = GenericRelation(Comment, object_id_field='object_pk')
